...
public:
  invalid_binary_format_error() :
    binary_error("invalid binary format") {
  }
};
...
public:
  explicit unsupported_section_type_error(const std::string& name) :
    section_error(
        "section '" + name + "' stores data of an unsupported type") {
  }
};
...
public:
  invalid_section_name_error() :
    section_error("section name too long") {
  }
};

class section_exists_error : public section_error {
public:
  section_exists_error() :
    section_error("section already exists") {
  }
};
